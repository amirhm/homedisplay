...
int display_time_update(time_t utc_time, datetime_t t, DayCnt day_cnt){
	...
	write_string(display.ML, 2 * 32 , string, fcolor, 1);
	//sprintf(string, "      %02d:%02d:%02d", t.hour , t.min, t.sec);
	sprintf(string, "  %02d:%02d", t.hour , t.min);
	write_string(display.ML, 32 , string, fcolor, 1);

	sprintf(string, " %02dW %dD ~", day_cnt.week, day_cnt.day);
	write_string(display.ML, 7 * 32 + 16 , string, fcolor, 1);
...
int display_info_update(SensorData weather){
	...
	write_string(display.ML, 4 * 32, string, (weather.co2_raw < 1200)? gcolor: rcolor, 1);
	sprintf(string, "TMP: %2.1f", weather.temperature);
	write_string(display.ML, 5 * 32, string, fcolor, 1);
	sprintf(string, "RH: %2.1f%%", weather.humidity);
	write_string(display.ML, 6 * 32, string, fcolor, 1);
